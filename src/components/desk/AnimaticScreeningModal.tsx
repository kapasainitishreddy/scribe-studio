...
import type { StoryboardPanel } from "../../../packages/project-model/src/types";
...
export const AnimaticScreeningModal: React.FC<AnimaticScreeningModalProps> = ({
  ...
  useEffect(() => {
    ...
    const updateFrames = (time: number) => {
      if (isPlaying && time - lastTime >= 1000 / 24) {
  ...
  }, [isPlaying, currentIndex, panels.length, panelDurationMs]);

  // Keyboard controls (Space = Play/Pause, Left/Right = Prev/Next, Esc = Close)
  useEffect(() => {
  ...
  }, [currentIndex, panels.length, onClose]);

  // Format 24 FPS timecode: 01:00:SS:FF
  ...
  // Determine dynamic camera motion CSS class
  ...
  }, [isPlaying, cameraMovement, panelDurationMs, currentIndex]);
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 flex items-center justify-center p-6 relative overflow-hidden bg-black">
        {/* Optical Cinema Vignette & Subtle Anamorphic Flare */}
        ...
        <div className="absolute inset-x-0 h-px top-1/2 bg-gradient-to-r from-transparent via-[#0EA5E9]/15 to-transparent pointer-events-none z-10" />
        ...
        >
          {/* Animated SVG Schematic / Artwork */}
          <div
          ...
          {/* Lens & Technical Specs Corner HUD */}
